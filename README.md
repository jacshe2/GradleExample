...
### 3. Add Task
Add the following to your build.gradle to run a task
...
```shell script
./gradlew build projectupper
```

Rather than running build each time, we can make our new task depend on a build

Replace the previous code with:
```groovy
task projectupper(dependsOn: 'build') {
    group = 'Custom Tasks'
    description = 'Task to get Project Name and Convert it to upper case'
    doLast {
        String projectName = project.name
...
also add the test dependencies to the build.gradle
```groovy
test {
...
create the following file called AppTest.java under src.test.java.org.example
```java
/*
...
Finally add the class to your previous created main class:
```java
package org.example;
